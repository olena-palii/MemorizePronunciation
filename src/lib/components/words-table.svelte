<script lang="ts">
    ...
    let { words, saveWord, deleteWord, playAudio, onDoubkeClick, search }: Props = $props();
...
<div class="overflow-x-hidden rounded-box border border-base-content/5 bg-base-100 h-96">
    ...
    <table class="table-sm table-pin-rows table-fixed w-full max-w-xl">
        ...
        <tr class="word-row hover:bg-base-300" ondblclick={() => { if (onDoubkeClick) onDoubkeClick(word); }}>
            ...
            <td>
                <button class="word-pronunciation btn btn-square btn-success" aria-label="Listen to pronunciation" onclick={() => (playAudio ?? playNativeAudio)(word.word)}>
