<script lang="ts">
  ...
  async function stop() {
    recordedAudio = await stopRecordingAudio();
    recording = false;
    recordedWord = word;
  }
...
<div class="card w-96 bg-base-100 shadow-sm">
  <div class="card-body">
    ...
    <div class="flex justify-center gap-8 mt-8">
      <button class="listen btn btn-circle btn-success btn-xl" aria-label="Listen to pronunciation" onclick={() => playNativeAudio(word.word)}>
      ...
      {:else}
        <button class="record btn btn-circle btn-error btn-xl" aria-label="Record your pronunciation" onclick={record}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 18.75a6 6 0 0 0 6-6v-1.5m-6 7.5a6 6 0 0 1-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 0 1-3-3V4.5a3 3 0 1 1 6 0v8.25a3 3 0 0 1-3 3Z" />
          </svg>
        </button>
      {/if}
      <button class="play btn btn-circle btn-warning btn-xl" aria-label="Play recorded pronunciation" onclick={ () => playRecordedAudio(recordedAudio) } disabled={!isRecorded()}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
